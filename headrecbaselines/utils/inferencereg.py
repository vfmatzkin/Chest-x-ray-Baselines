...
def fit_meshes_to_vol(preds_folder, vols_folder, pred_suffix='', vol_suffix='',
                      overwrite=False):
    ...
    for pred in os.listdir(preds_folder):
        if not pred.endswith(pred_suffix):
            continue
        pred_path = os.path.join(preds_folder, pred)
        vol_path = os.path.join(vols_folder, pred.replace(pred_suffix,
                                                          vol_suffix))
